...
// Ajusta este tipo a tu interfaz real de usuario en tu app
...
export const useAuthStore = create<AuthState>()((set, get) => ({
  ...
  loadSession: async () => {
    try {
      ...
      if (sessionData) {
        const { user, userType, userName, token } = JSON.parse(sessionData);
        set({
          status: 'authenticated',
          user,
          userType,
  ...
  logout: async () => {
    try {
      ...
      try {
        localStorage.removeItem('lexyvoz:lastPath');
      } catch {}
    } finally {
      set({
        status: 'unauthenticated',
  ...
  updateUser: async (updatedFields: Partial<User>) => {
    ...
    try {
      // Nota: ignoramos el id aquí; el backend debe inferir por token en /auth/profile
